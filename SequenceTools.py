...
class SequenceTools:
    ...
    def returnFirstCodingSequence(self, seq):
        start = None
        for i in range(0, len(seq)):
            subSeq = seq[i:i+3]
            peptide = subSeq.translate(CodonTable.unambiguous_dna_by_id[1])
            if str(peptide) == "M":
                start = i
                break
        startToEnd = seq[start:]
        startToEndPeptideSeq = startToEnd.translate(CodonTable.unambiguous_dna_by_id[1])
        endPeptide = None
        for i in range(0, len(startToEndPeptideSeq)):
            if startToEndPeptideSeq[i] == "*":
                endPeptide = i
                break
        peptideDNASeq = startToEnd[:(1+endPeptide)*3]
        return peptideDNASeq

    def deconstructImportedCDNASequence(self, sequence, sequenceIdentifier):
        seq = Seq(sequence)
        codingSeq = self.returnFirstCodingSequence(seq)
